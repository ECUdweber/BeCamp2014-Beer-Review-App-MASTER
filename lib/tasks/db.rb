...
db_namespace = namespace :db do
  ...
  def current_config(options = {})
    options = { :env => App.env }.merge! options
  ...
  task :load_config => :environment do
    #ActiveRecord::Base.configurations = App.application.config.database_configuration
    ActiveRecord::Migrator.migrations_paths = App.root.join('db/migrate')
  end
  ...
  desc 'Create the database from DATABASE_URL or config/database.yml for the current App.env (use db:create:all to create all dbs in the config)'
  ...
  desc 'Drops the database using DATABASE_URL or the current App.env (use db:drop:all to drop all databases)'
  task :drop => [:load_config, :environment] do
    if ENV['DATABASE_URL']
  ...
  task :charset => [:environment, :load_config] do
    config = ActiveRecord::Base.configurations[App.env]
    case config['adapter']
    when /mysql/
      ActiveRecord::Base.establish_connection(config)
  ...
  namespace :fixtures do
    ...
    task :load => [:environment, :load_config] do
      ...
      base_dir     = File.join [App.root, ENV['FIXTURES_PATH'] || %w{test fixtures}].flatten
  ...
  namespace :schema do
    ...
    task :dump => [:environment, :load_config] do
      ...
      filename = ENV['SCHEMA'] || "#{App.root}/db/schema.rb"
    ...
    task :load => [:environment, :load_config] do
      file = ENV['SCHEMA'] || "#{App.root}/db/schema.rb"
      if File.exists?(file)
        load(file)
      else
        abort %{#{file} doesn't exist yet. Run `rake db:migrate` to create it then try again. If you do not intend to use a database, you should instead alter #{App.root}/config/application.rb to limit the frameworks that will be loaded}
      end
    end
  ...
  namespace :structure do
    ...
    task :dump => [:environment, :load_config] do
      ...
      filename = ENV['DB_STRUCTURE'] || File.join(App.root, "db", "structure.sql")
      case config['adapter']
...
def drop_database(config)
  ...
  when /sqlite/
    ...
    file = path.absolute? ? path.to_s : File.join(App.root, path)
...
def configs_for_environment
  ...
  environments << 'test' if App.env.development?
